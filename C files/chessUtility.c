...
Dependancies: NONE
*/
ChessBoardType **fillBoard( ChessBoardType **board, int rows, int cols )
   {
   ...
   for( rowIndex = 0; rowIndex < rows; rowIndex++ )
      ...
      for( colIndex = 0; colIndex < cols; colIndex++ )
         ...
         board[ rowIndex ][ colIndex ].type = initialPieces[ rowIndex * cols + colIndex ];
...
*/
ChessBoardType **initializeChessBoard( int rows, int cols )
   {
   // iniitlaize functions/variables
       ChessBoardType **newBoard;
       int index;

   newBoard = ( ChessBoardType ** )malloc( rows * sizeof( ChessBoardType * ) );

   for( index = 0; index < rows; index++ )
      {

      newBoard[ index ] = ( ChessBoardType * )malloc
                                           ( cols * sizeof( ChessBoardType ) );
      }
